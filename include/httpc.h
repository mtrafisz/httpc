...
typedef enum _httpc_error_e {
    HTTPC_NO_ERROR = 0,
    HTTPC_NOT_ENOUGH_DATA,
    HTTPC_MALFORMED_DATA,
    HTTPC_INVALID_METHOD,
    // TODO: fill up as needed
} httpc_error_t;
...
httpc_error_t httpc_request_deserialize(const array_t(byte) * data, httpc_request_t* out_request);  // todo: error enum?
...
httpc_error_t httpc_response_deserialize(const array_t(byte) * data, httpc_response_t* out_response);
